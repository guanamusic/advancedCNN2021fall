...
from tools import torch_nearest_upsampler, torch_nearest_downsampler, same_padding, \
    extract_image_patches, reduce_mean, reduce_sum, flow_to_image


"""
The below is referred by daa233's repo (with minor fix).
(https://github.com/daa233/generative-inpainting-pytorch)
"""

class ContextualAttention(BaseModule):
    ...
    def forward(self, f, b, mask=None):
        ...
        f = torch_nearest_downsampler(f, factor=self.rate)
        # b = F.interpolate(b, scale_factor=1./self.rate, mode='nearest')
        b = torch_nearest_downsampler(b, factor=self.rate)
        ...
        if mask is None:
            ...
            mask = torch_nearest_downsampler(mask, factor=4*self.rate)
        ...
        if self.rate != 1:
            ...
            flow = torch_nearest_upsampler(flow, factor=self.rate*4)
